<#
.SYNOPSIS
    Desktop Management Logon Script - Windows 10
    
.DESCRIPTION
    Main entry point for Desktop Management Suite during user logon on Windows 10.
    Uses workflow engine to execute steps defined in Config\Workflow-Logon.psd1
...
.EXAMPLE
    ...
    .\DesktopManagement-Logon.ps1 -VerboseLogging -MaxLogAge 30
    
.NOTES
    Version: 2.0.0
    Replacement for: VB/DesktopManagement.wsf //Job:GDP_10_Logon
    
    To add/remove/reorder actions:
    - Edit Config\Workflow-Logon.psd1
...
Try {
    ...
    # Import service modules (needed by mappers and inventory)
    Import-Module "$PSScriptRoot\Modules\Services\DMServiceCommon.psm1" -Force -ErrorAction Stop
    Import-Module "$PSScriptRoot\Modules\Services\DMMapperService.psm1" -Force -ErrorAction Stop
    Import-Module "$PSScriptRoot\Modules\Services\DMInventoryService.psm1" -Force -ErrorAction Stop
    
    # Import inventory modules (needed by Drive Mapper for Get-DMMappedDrives)
    Import-Module "$PSScriptRoot\Modules\Inventory\Invoke-DriveInventory.psm1" -Force -ErrorAction Stop
...
Write-DMLog "Desktop Management Suite - Logon Script" -NoTimestamp
...
Try {
    ...
    If ($Computer.Groups.Count -gt 0) {
        $Computer.Groups | ForEach-Object { Write-DMLog "    $($_.DistinguishedName)" -Level Info }
    }
    Write-DMLog ""
    ...
    If ($User.Groups.Count -gt 0) {
        $User.Groups | ForEach-Object { Write-DMLog "    $($_.DistinguishedName)" -Level Info }
    }
    Write-DMLog ""
} Catch {
...
# ============================================================================
Write-DMLog "========================================" -NoTimestamp
Write-DMLog "Desktop Management Logon - Completed" -NoTimestamp
Write-DMLog "========================================" -NoTimestamp
