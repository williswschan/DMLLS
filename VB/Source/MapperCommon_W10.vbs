'This module contains classes and functions that are shared by Mapper modules
...
'Find best available MapperServer for the computer
Function GetMapperServer(strComputerDomain)
	...
	strServerName 	= Trim(getResource("MapperServer"))
	strServiceName 	= Trim(getResource("MapperService"))
	...
	Set objMapperServer = New MapperServer
	
	objMapperServer.ServerName = strServerFqdn
	objMapperServer.ServiceURL = "http://" & strServerFqdn & "/" & strServiceName
	...
	If (Err.Number = 0) Then
		For Each objPingStatus in colPing
			If (objPingStatus.StatusCode = 0) Then
				objMapperServer.Online = True
				objMapperServer.ResponseTime = objPingStatus.ResponseTime
			Else
				objMapperServer.Online = False
			End If
		Next
	Else
		Err.Clear
	...
	If (objMapperServer.Online) Then
		Set objHttp = CreateObject("Msxml2.XMLHTTP.3.0")
		
		objHttp.Open "POST", objMapperServer.ServiceURL, True
			...
			If (intTimeOut > 0) Then
				If (objHttp.Status = 200) Then
					...
					If (StrComp(Trim(objHttp.ResponseXML.Text), "True", 1) = 0) Then
						objMapperServer.ServiceAvailable = True						
					End If
				End If
			End If
		Else
		...
		If (objMapperServer.ServiceAvailable) Then
			'Return most responsive and available MapperServer and exit function
			Set GetMapperServer = objMapperServer
			Exit Function
		End If	
	Else
		'No available service was found
		Set GetMapperServer = Nothing
	End If		
...
Function IsOfflineLaptopMapper(OfflineLaptopGroup)
...
Function IsHostPartOfGroup(strGroupName)
	...
	Set objGroup = GetObject("LDAP://CN=" & strGrpName & ",OU=Access Control,OU=Groups,OU=Regional Management,DC=" & Main.Computer.ShortDomain & ",DC=NOM")
	
	If Err.Number <> 0 Then
		retVal = False
	Else
		GetMembers "LDAP://CN=" & strGrpName & ",OU=Access Control,OU=Groups,OU=Regional Management,DC=" & Main.Computer.ShortDomain & ",DC=NOM", dicSeenGroupMember, HostList
		
		If (HostList.Exists("LDAP://"  & Main.Computer.DN)) Then
			retVal = True
		End If
		
		dicSeenGroupMember.RemoveAll
		HostList.RemoveAll
	End If
	
	On Error GoTo 0
	
	IsHostPartOfGroup = retVal
	
End Function

Function GetMembers (strGroupADsPath, dicSeenGroupMember, UserList)
	 ...
	 For Each objMember In objGroup.Members
	  If objMember.Class = "group" Then
			...
			Else
				...
				GetMembers objMember.ADsPath, dicSeenGroupMember, UserList
			End If
	   Else
